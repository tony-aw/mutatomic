...
#' \cr
#' `currentBindings(x, action = "lock")` \cr
#' searches all \bold{currently existing} objects
#' sharing the same \bold{address} as `x`, in a given environment,
#' and locks them using \link[base]{lockBinding}. \cr
#' \cr
#' See also \link{mutatomic_PassByReference} for information regarding
...
#' 'mutatomic in principle respect this,
...
#' But since `ma_set()` performs modification by reference,
...
#' \cr
#' For `currentBindings(x, action = "lock")`: \cr
...
#'
#'
#' @example inst/examples/currentBindings.R
#' 
#' 
#' 
